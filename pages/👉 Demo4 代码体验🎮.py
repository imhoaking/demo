...
option1 = st.sidebar.checkbox('Eetric状态',1,0)
if option1:
  with st.expander("Etric状态"):
    st.metric(label="状态", value="正能量", delta="120% 😄",help='哈哈没啥')


option2 = st.sidebar.checkbox('输出json',1,0)
if option2:
...
option3 = st.sidebar.checkbox('固定数',1,0)
if option3:
...
option4 = st.sidebar.checkbox('随机数',1,0)
if option4:
...
option5 = st.sidebar.checkbox('代码块高亮',1,0)
if option5:
...
option6 = st.sidebar.checkbox('进度条',1,0)
if option6:
